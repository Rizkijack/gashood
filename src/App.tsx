...
// ─── Lazy 3D scene (dibundle chunk terpisah) ──────────────────────────
...
export default function App() {
  ...
  // kalau store masih kosong (guard di seedFromSnapshot). Refresh berkala
  // cukup agresif — sejalan dengan cadence collector Actions (±1 jam).
  useEffect(() => {
    let cancelled = false;
    const hydrate = async () => {
    ...
    hydrate();
    const interval = setInterval(hydrate, 10 * 60 * 1000);
    return () => {
      cancelled = true;
